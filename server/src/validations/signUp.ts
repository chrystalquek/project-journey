...
const status = body('status').exists().withMessage('Status is required').custom((status: string) => stringEnumValidator(SIGN_UP_STATUS, 'Sign Up Status', status)).withMessage('Status is invalid');
